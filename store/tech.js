...
const tech = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "TYPESCRIPT":
      ...
      VSC와 함께 사용할때 개발이 편해지고 에러를 방지해줘서 왠만하면 자바스크립트보다 사용하고 싶은 언어입니다.
      `;
    case "REACT":
    ...
    case "REACTNATIVE":
      ...
      앱을 위한 모듈의 문서를 읽으면서 공부하고 주말의 뭐하지 라는 앱을 클론코딩하면서 익혀보았습니다. 
      `;
    case "NEXTJS":
      return `Nextjs는 서버 사이드 렌더링을 위한 엄청난 라이브러리 입니다. 
      이번 포트폴리오도 nextjs를 사용 하였으며 빠른 반응을 제공하기 위해 사용하였습니다.`;
    case "APOLLO":
      return `아폴로 클라이언트는 Graphql을 클라이언트에서 사용하기 위해 편의성을 제공하는 라이브러리 입니다.`;
    case "REDUX":
      return `저는 ContextAPI 다루는데 익숙하나 애플리케이션이 커지면서 생기는 props의 전달과정에서 불편함을 느끼고 Redux를 공부하였습니다.
        이번 포트폴리오는 작은 크기이지만 Redux를 사용해 익혀보았습니다.
      `;
    case "HTML5":
      return `
    ...
    case "CSS":
      return `
      css는 여러 페이지를 따라 만들어 보면서 연습하였고, stled-components라는 모듈을 사용해서 사용하는 것을 좋아합니다.
